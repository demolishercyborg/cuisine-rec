...
```bash
git clone https://github.com/demolishercyborg/cuisine-rec.git
cd cuisine-engine
```
...
The **backend** container runs FastAPI on port 8000, but that port is not published to your host. It's only reachable from the frontend container over Docker's internal network.
